<? include $_SERVER['DOCUMENT_ROOT'] . '/pages/top2.php' ?>

<!-- main -->
<main class="page page_second">
    <? include $_SERVER['DOCUMENT_ROOT'] . '/pages/blocks/aside.php' ?>
    <main class="main">
        <div class="port wood">
            <section class="port-lap">
                <div class="port-lap__bgvideo">
                    <video autoplay muted loop class="port-lap__bgvideo-video"
                           src="/images/content/portfolio/project15/videoplayback.mp4"
                           poster="/images/content/portfolio/project15/bg.jpg">
                        <source src="/images/content/portfolio/project15/videoplayback.mp4" type="video/mp4">
                    </video>
                </div>
                <div class="container3">
                    <div class="port-lap__bread">
                        <div class="bread">
                            <a class="bread__item" href="/">Создание сайта</a>
                            <a class="bread__item" href="/portfolio/">Портфолио</a>
                            <a class="bread__item active">Woodland.ru</a>
                        </div>
                    </div>

                    <div class="port-lap__wrapper">
                        <div class="proportion proportion_1214x685"></div>
                        <div class="port-lap__wrap">
                            <div class="port-lap__container">
                                <div class="proportion proportion_750x470"></div>
                                <div class="port-lap__before">
                                    <div class="port-lap__drug draggable"></div>
                                    <div class="port-lap__before-wrap"><img class="port-lap__img"
                                                                            src="/images/content/portfolio/project15/before.jpg">
                                    </div>
                                </div>
                                <div class="port-lap__after"><img class="port-lap__img"
                                                                  src="/images/content/portfolio/project15/after.jpg">
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="port-lap__bottom"></div>

                </div>
            </section>
            <div class="container3">

                <!-- desc -->
                <div class="port-desc">
                    <!-- logo -->
                    <div class="port-desc__logo">
                        <img class="port-desc__logo-img" src="/images/content/portfolio/project15/logo.png"></div>
                    <!-- END logo -->

                    <!-- title -->
                    <div class="port-desc__title">
                        Разработка сайта строительной тематики<br>
                        с детальным каталогом работ
                    </div>
                    <!-- END title -->

                    <!-- subtitle -->
                    <div class="port-desc__sub">
                        Реализация "под ключ"<br>
                        с учётом индивидуальных особенностей проекта
                    </div>
                    <!-- END subtitle -->

                    <!-- numbers -->
                    <!-- Для изменнения времени смены цифр необходимо менять дата атрибут data-timer -->
                    <div class="port-desc__list numbers" data-timer="1000">
                        <div class="port-desc__item number">
                            <div class="port-desc__item-title">
                                <span class="number__val port-desc__item-val" data-from="0" data-to="6">
                                    0
                                </span>
                            </div>
                            <div class="port-desc__item-text">прототипов</div>
                        </div>
                        <div class="port-desc__item number">
                            <div class="port-desc__item-title">
                                <span class="number__val port-desc__item-val" data-from="0" data-to="42">
                                    0
                                </span>
                            </div>
                            <div class="port-desc__item-text">дизайн-макета</div>
                        </div>
                        <div class="port-desc__item number">
                            <div class="port-desc__item-title">
                                <span class="number__val port-desc__item-val" data-from="0" data-to="36">
                                    0
                                </span>
                            </div>
                            <div class="port-desc__item-text">адаптивных стр.</div>
                        </div>
                        <div class="port-desc__item number">
                            <div class="port-desc__item-title">
                                <span class="number__val port-desc__item-val" data-from="0" data-to="56">
                                    0
                                </span>
                                <span class="port-desc__item-type"> дней</span>
                            </div>
                            <div class="port-desc__item-text">время работы</div>
                        </div>
                    </div>
                    <!-- END numbers -->

                </div>
                <!-- END desc -->

                <!-- slider -->
                <div class="port-slider">

                    <!-- top -->
                    <div class="port-slider__top">
                        <div class="port-slider__title">Яркие<br>баннеры</div>
                        <div class="port-slider__desc">Акценты позволяют выделить важную<br>информацию и оставить
                            положительные<br>эмоции от использования сайта
                        </div>
                        <div class="port-slider__nav">
                            <div class="port-slider__prev">
                                <div class="arrow_prev arrow_black arrow">
                                    <div class="arrow__line"></div>
                                    <div class="arrow__triangle"></div>
                                </div>
                            </div>
                            <div class="port-slider__pagination"></div>
                            <div class="port-slider__next">
                                <div class="arrow_black arrow">
                                    <div class="arrow__line"></div>
                                    <div class="arrow__triangle"></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <!-- END top -->

                    <!-- slider -->
                    <div class="port-slider__slider">
                        <div class="swiper-container port-slider__container">
                            <div class="swiper-wrapper">
                                <div class="swiper-slide port-slider__slide">
                                    <img class="port-slider__img" src="/images/content/portfolio/project15/slide1.jpg">
                                </div>
                                <div class="swiper-slide port-slider__slide">
                                    <img class="port-slider__img" src="/images/content/portfolio/project15/slide2.jpg">
                                </div>
                                <div class="swiper-slide port-slider__slide">
                                    <img class="port-slider__img" src="/images/content/portfolio/project15/slide3.jpg">
                                </div>
                                <div class="swiper-slide port-slider__slide">
                                    <img class="port-slider__img" src="/images/content/portfolio/project15/slide4.jpg">
                                </div>
                                <div class="swiper-slide port-slider__slide">
                                    <img class="port-slider__img" src="/images/content/portfolio/project15/slide5.jpg">
                                </div>
                                <div class="swiper-slide port-slider__slide">
                                    <img class="port-slider__img" src="/images/content/portfolio/project15/slide6.jpg">
                                </div>
                                <div class="swiper-slide port-slider__slide">
                                    <img class="port-slider__img" src="/images/content/portfolio/project15/slide7.jpg">
                                </div>
                            </div>
                        </div>
                    </div>
                    <!-- END slider -->

                    <!-- bottom -->
                    <div class="port-slider__bottom"></div>
                    <!-- END bottom -->

                </div>
                <!-- END slider -->

                <!-- project -->
                <div class="port-proj port-proj_1">

                    <!-- left -->
                    <div class="port-proj__left">
                        <div class="port-proj__img">
                            <img class="port-proj__img-img" src="/images/content/portfolio/project15/project.jpg">
                        </div>
                    </div>
                    <!-- END left -->

                    <!-- right -->
                    <div class="port-proj__right">

                        <!-- block 1-->
                        <div class="port-proj__block">
                            <!-- title -->
                            <div class="port-proj__title">
                                ГЛАВНАЯ
                                СТРАНИЦА
                            </div>
                            <!-- END title -->
                        </div>
                        <!-- END block 1-->

                        <!-- block 2 -->
                        <div class="port-proj__block">
                            <!-- list -->
                            <div class="port-proj__list">
                                <div class="port-proj__item">
                                    <div class="port-proj__item-title">виды услуг
                                        <div class="port-proj__item-line"></div>
                                    </div>
                                    <div class="port-proj__item-desc">
                                        Превью услуг, красиво оформлены тематическими фотографиями
                                    </div>
                                </div>
                                <div class="port-proj__item">
                                    <div class="port-proj__item-title">проекты
                                        <div class="port-proj__item-line"></div>
                                    </div>
                                    <div class="port-proj__item-desc">
                                        Карточки проектов с формой взаимодействия 
                                    </div>
                                </div>
                                <div class="port-proj__item">
                                    <div class="port-proj__item-title">о компании
                                        <div class="port-proj__item-line"></div>
                                    </div>
                                    <div class="port-proj__item-desc">
                                        Портфолио и краткое описание компании, достоинства, преимущества и некоторые факты
                                    </div>
                                </div>
                            </div>
                            <!-- END list -->
                        </div>
                        <!-- END block 2 -->

                        <!-- block 3 -->
                        <div class="port-proj__block">
                        </div>
                        <!-- END block 3 -->

                    </div>
                    <!-- END right -->

                </div>
                <!-- END project -->

                <!-- project 2 -->
                <div class="port-proj port-proj_2">

                    <!-- left -->
                    <div class="port-proj__left">
                        <div class="port-proj__img">
                            <img class="port-proj__img-img" src="/images/content/portfolio/project15/project2.jpg">
                        </div>
                    </div>
                    <!-- END left -->

                    <!-- right -->
                    <div class="port-proj__right">

                        <!-- block 1-->
                        <div class="port-proj__block">
                            <!-- title -->
                            <div class="port-proj__title">
                                страница
                                категорий
                            </div>
                            <!-- END title -->
                        </div>
                        <!-- END block 1-->

                        <!-- block 2 -->
                        <div class="port-proj__block">
                            <!-- list -->
                            <div class="port-proj__list">
                                <div class="port-proj__item">
                                    <div class="port-proj__item-title">Типы проектов
                                        <div class="port-proj__item-line"></div>
                                    </div>
                                    <div class="port-proj__item-desc">
                                        Удобные карточки с фотографиями различных типов, поиском через фильтр и сортировкой по цене
                                    </div>
                                </div>
                                <div class="port-proj__item">
                                    <div class="port-proj__item-title">SEO описание
                                        <div class="port-proj__item-line"></div>
                                    </div>
                                    <div class="port-proj__item-desc">
                                        Детальное описание страницы позволяет сайту успешно продвигаться в поиске
                                    </div>
                                </div>
                            </div>
                            <!-- END list -->
                        </div>
                        <!-- END block 2 -->

                        <!-- block 3 -->
                        <div class="port-proj__block"></div>
                        <!-- END block 3 -->

                    </div>
                    <!-- END right -->

                </div>
                <!-- END project 2 -->

                <!-- project 3 -->
                <div class="port-proj port-proj_3">

                    <!-- left -->
                    <div class="port-proj__left">
                        <div class="port-proj__img">
                            <img class="port-proj__img-img" src="/images/content/portfolio/project15/project3.jpg">
                        </div>
                    </div>
                    <!-- END left -->

                    <!-- right -->
                    <div class="port-proj__right">

                        <!-- block 1-->
                        <div class="port-proj__block">
                            <!-- title -->
                            <div class="port-proj__title">
                                страница
                                проекта
                            </div>
                            <!-- END title -->
                        </div>
                        <!-- END block 1-->

                        <!-- block 2 -->
                        <div class="port-proj__block">
                            <!-- list -->
                            <div class="port-proj__list">
                                <div class="port-proj__item">
                                    <div class="port-proj__item-title">карточка проекта
                                        <div class="port-proj__item-line"></div>
                                    </div>
                                    <div class="port-proj__item-desc">
                                        Информация о проекте, размеры, материалы, планировка, характеристики.
                                    </div>
                                </div>
                                <div class="port-proj__item">
                                    <div class="port-proj__item-title">схема
                                        <div class="port-proj__item-line"></div>
                                    </div>
                                    <div class="port-proj__item-desc">
                                        Графическая схема, разработанная в специальной програме
                                    </div>
                                </div>
                                <div class="port-proj__item">
                                    <div class="port-proj__item-title">обратная связь
                                        <div class="port-proj__item-line"></div>
                                    </div>
                                    <div class="port-proj__item-desc">
                                        Можно связаться с менеджером и рассчитать стоимость
                                    </div>
                                </div>
                            </div>
                            <!-- END list -->
                        </div>
                        <!-- END block 2 -->

                        <!-- block 3 -->
                        <div class="port-proj__block">
                        </div>
                        <!-- END block 3 -->

                    </div>
                    <!-- END right -->

                </div>
                <!-- END project 3 -->

                <!-- project 4 -->
                <div class="port-proj port-proj_4">

                    <!-- left -->
                    <div class="port-proj__left">
                        <div class="port-proj__img">
                            <img class="port-proj__img-img" src="/images/content/portfolio/project15/project4.jpg">
                        </div>
                    </div>
                    <!-- END left -->

                    <!-- right -->
                    <div class="port-proj__right">

                        <!-- block 1-->
                        <div class="port-proj__block">
                            <!-- title -->
                            <div class="port-proj__title">
                                страница
                                о компании
                            </div>
                            <!-- END title -->
                        </div>
                        <!-- END block 1-->

                        <!-- block 2 -->
                        <div class="port-proj__block">
                            <!-- list -->
                            <div class="port-proj__list">
                                <div class="port-proj__item">
                                    <div class="port-proj__item-title">история
                                        <div class="port-proj__item-line"></div>
                                    </div>
                                    <div class="port-proj__item-desc">
                                        Преимущества и миссия компании
                                    </div>
                                </div>
                                <div class="port-proj__item">
                                    <div class="port-proj__item-title">достоинства
                                        <div class="port-proj__item-line"></div>
                                    </div>
                                    <div class="port-proj__item-desc">
                                        Причины по котрым доверяют и обращаются именно к данной компании
                                    </div>
                                </div>
                            </div>
                            <!-- END list -->
                        </div>
                        <!-- END block 2 -->

                    </div>
                    <!-- END right -->

                </div>
                <!-- END project 4 -->

                <!-- pages -->
                <div class="port-pages">
                    <div class="port-pages__title">
                        внутренние
                        страницы
                    </div>
                    <div class="port-pages__imgs">
                        <img class="port-pages__img" src="/images/content/portfolio/project15/pages.png"></div>

                </div>
                <div class="port-pages-bottom"></div>
                <!-- END pages -->

                <!-- team -->
                <div class="port-team">
                    <div class="port-team__title">Команда<br>проекта</div>
                    <div class="port-team__img">
                        <img class="port-team__img-img" src="/images/content/portfolio/team.png">
                    </div>
                    <div class="port-team__list">
                        <div class="port-team__col">
                            <div class="port-team__item">
                                <div class="port-team__item-title">Фёдоров Юрий</div>
                                <div class="port-team__item-prof">Менеджер проекта</div>
                            </div>
                            <div class="port-team__item">
                                <div class="port-team__item-title">Ефремов Денис</div>
                                <div class="port-team__item-prof">Дизайнер</div>
                            </div>
                            <div class="port-team__item">
                                <div class="port-team__item-title">Серёгин Михаил</div>
                                <div class="port-team__item-prof">Front-end разработчик</div>
                            </div>
                        </div>
                        <div class="port-team__col">
                            <div class="port-team__item">
                                <div class="port-team__item-title">Соловьев Михаил</div>
                                <div class="port-team__item-prof">Back-end разработчик</div>
                            </div>
                            <div class="port-team__item">
                                <div class="port-team__item-title">Соколов Александр</div>
                                <div class="port-team__item-prof">Тестировщик</div>
                            </div>
                        </div>
                        <div class="port-team__col">
                            <div class="port-team__item">
                                <div class="port-team__item-title">Рожанский Виктор</div>
                                <div class="port-team__item-prof">Контент менеджер</div>
                            </div>
                            <div class="port-team__item">
                                <div class="port-team__item-title">Рябова Наталья</div>
                                <div class="port-team__item-prof">Маркетолог</div>
                            </div>
                        </div>
                    </div>
                </div>
                <!-- END team -->

            </div>
            <section class="port-bottom">
                <div class="bottom">
                    <div class="container2">
                        <div class="bottom__send">
                            <? include $_SERVER['DOCUMENT_ROOT'] . '/pages/blocks/bottom/send.php' ?>
                        </div>
<div class="why">
    <div class="why__img"><img class="why__img-img" src="/images/content/why.png"></div>
    <div class="why__title">Работаем на результат</div>
    <div class="why__content">
        <p>Для нас важно создать проект и привести его в идеально состояние. Мы заинтересованы создавать те проекты, которые нам не стыдно показать. К сожалению, в некоторых проектах мы не можем проявлять свой талант и креативный подход в полном объеме, так как отталкиваемся от жёстких требований заказчиков по цвету, расположению элементов на странице, 
		эффектов и размещению информации. Обращаясь к нам, вы получаете команду специалистов с большим опытом работы. Для нас нет невыполнимых задач, мы возьмемся за самые смелые ваши идеи и доведём их до финального этапа.</p>
		<br>
	</div>
</div>
                        <div class="bottom__footer">
                            <? include $_SERVER['DOCUMENT_ROOT'] . '/pages/blocks/bottom/footer.php' ?>
                        </div>
                    </div>
                </div>
            </section>
        </div>
    </main>
</main>
<!-- END main -->

<? include $_SERVER['DOCUMENT_ROOT'] . '/pages/bottom.php' ?>
