<? include $_SERVER['DOCUMENT_ROOT'] . '/pages/top2.php' ?>

<!-- main -->
<main class="page page_second">
    <? include $_SERVER['DOCUMENT_ROOT'] . '/pages/blocks/aside.php' ?>
    <main class="main">
        <div class="port clinic">
            <section class="port-lap">
                <div class="port-lap__bgvideo">
                    <video autoplay muted loop class="port-lap__bgvideo-video"
                           src="/images/content/portfolio/project14/videoplayback.mp4"
                           poster="/images/content/portfolio/project14/bg.jpg">
                        <source src="/images/content/portfolio/project14/videoplayback.mp4" type="video/mp4">
                    </video>
                </div>
                <div class="container3">
                    <div class="port-lap__bread">
                        <div class="bread">
                            <a class="bread__item" href="/">Создание сайтов</a>
                            <a class="bread__item" href="/portfolio/">Портфолио</a>
                            <a class="bread__item active">Империя стоматология</a>
                        </div>
                    </div>

                    <div class="port-lap__wrapper">
                        <div class="proportion proportion_1214x685"></div>
                        <div class="port-lap__wrap">
                            <div class="port-lap__container">
                                <div class="proportion proportion_750x470"></div>
                                <div class="port-lap__before">
                                    <div class="port-lap__drug draggable"></div>
                                    <div class="port-lap__before-wrap"><img class="port-lap__img"
                                                                            src="/images/content/portfolio/project14/before.jpg">
                                    </div>
                                </div>
                                <div class="port-lap__after"><img class="port-lap__img"
                                                                  src="/images/content/portfolio/project14/after.jpg">
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="port-lap__bottom"></div>

                </div>
            </section>
            <div class="container3">

                <!-- desc -->
                <div class="port-desc">
                    <!-- logo -->
                    <div class="port-desc__logo">
                        <img class="port-desc__logo-img" src="/images/content/portfolio/project14/logo.png"></div>
                    <!-- END logo -->

                    <!-- title -->
                    <div class="port-desc__title">
                        Разработка сайта для стомотологической клиники, с наполнением "под ключ"
                    </div>
                    <!-- END title -->

                    <!-- subtitle -->
                    <div class="port-desc__sub">
                        Настройка функционала онлайн записи, связь акций и услуг, привязка карточки доктора к услугам и акциям. 
                    </div>
                    <!-- END subtitle -->

                    <!-- numbers -->
                    <!-- Для изменнения времени смены цифр необходимо менять дата атрибут data-timer -->
                    <div class="port-desc__list numbers" data-timer="1000">
                        <div class="port-desc__item number">
                            <div class="port-desc__item-title">
                                <span class="number__val port-desc__item-val" data-from="0" data-to="7">
                                    0
                                </span>
                            </div>
                            <div class="port-desc__item-text">прототипов</div>
                        </div>
                        <div class="port-desc__item number">
                            <div class="port-desc__item-title">
                                <span class="number__val port-desc__item-val" data-from="0" data-to="16">
                                    0
                                </span>
                            </div>
                            <div class="port-desc__item-text">дизайн-макетов</div>
                        </div>
                        <div class="port-desc__item number">
                            <div class="port-desc__item-title">
                                <span class="number__val port-desc__item-val" data-from="0" data-to="32">
                                    0
                                </span>
                            </div>
                            <div class="port-desc__item-text">адаптивных стр.</div>
                        </div>
                        <div class="port-desc__item number">
                            <div class="port-desc__item-title">
                                <span class="number__val port-desc__item-val" data-from="0" data-to="35">
                                    0
                                </span>
                                <span class="port-desc__item-type"> дней</span>
                            </div>
                            <div class="port-desc__item-text">время работы</div>
                        </div>
                    </div>
                    <!-- END numbers -->

                </div>
                <!-- END desc -->

                <!-- slider -->
                <div class="port-slider">

                    <!-- top -->
                    <div class="port-slider__top">
                        <div class="port-slider__title">Яркие<br>баннеры</div>
                        <div class="port-slider__desc">Акценты позволяют выделить важную<br>информацию и оставить
                            положительные<br>эмоции от использования сайта
                        </div>
                        <div class="port-slider__nav">
                            <div class="port-slider__prev">
                                <div class="arrow_prev arrow_black arrow">
                                    <div class="arrow__line"></div>
                                    <div class="arrow__triangle"></div>
                                </div>
                            </div>
                            <div class="port-slider__pagination"></div>
                            <div class="port-slider__next">
                                <div class="arrow_black arrow">
                                    <div class="arrow__line"></div>
                                    <div class="arrow__triangle"></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <!-- END top -->

                    <!-- slider -->
                    <div class="port-slider__slider">
                        <div class="swiper-container port-slider__container">
                            <div class="swiper-wrapper">
                                <div class="swiper-slide port-slider__slide">
                                    <img class="port-slider__img" src="/images/content/portfolio/project14/slide1.jpg">
                                </div>
                                <div class="swiper-slide port-slider__slide">
                                    <img class="port-slider__img" src="/images/content/portfolio/project14/slide2.jpg">
                                </div>
                                <div class="swiper-slide port-slider__slide">
                                    <img class="port-slider__img" src="/images/content/portfolio/project14/slide3.jpg">
                                </div>
                                <div class="swiper-slide port-slider__slide">
                                    <img class="port-slider__img" src="/images/content/portfolio/project14/slide4.jpg">
                                </div>
                                <div class="swiper-slide port-slider__slide">
                                    <img class="port-slider__img" src="/images/content/portfolio/project14/slide5.jpg">
                                </div>
                                <div class="swiper-slide port-slider__slide">
                                    <img class="port-slider__img" src="/images/content/portfolio/project14/slide6.jpg">
                                </div>

                            </div>
                        </div>
                    </div>
                    <!-- END slider -->

                    <!-- bottom -->
                    <div class="port-slider__bottom"></div>
                    <!-- END bottom -->

                </div>
                <!-- END slider -->

                <!-- project -->
                <div class="port-proj port-proj_1">

                    <!-- left -->
                    <div class="port-proj__left">
                        <div class="port-proj__img">
                            <img class="port-proj__img-img" src="/images/content/portfolio/project14/project.jpg">
                        </div>
                    </div>
                    <!-- END left -->

                    <!-- right -->
                    <div class="port-proj__right">

                        <!-- block 1-->
                        <div class="port-proj__block">
                            <!-- title -->
                            <div class="port-proj__title">
                                ГЛАВНАЯ
                                СТРАНИЦА
                            </div>
                            <!-- END title -->
                        </div>
                        <!-- END block 1-->

                        <!-- block 2 -->
                        <div class="port-proj__block">
                            <!-- list -->
                            <div class="port-proj__list">
                                <div class="port-proj__item">
                                    <div class="port-proj__item-title">виды услуг
                                        <div class="port-proj__item-line"></div>
                                    </div>
                                    <div class="port-proj__item-desc">
                                        Список услуг с вложенными разделами
                                    </div>
                                </div>
                                <div class="port-proj__item">
                                    <div class="port-proj__item-title">сотрудники
                                        <div class="port-proj__item-line"></div>
                                    </div>
                                    <div class="port-proj__item-desc">
                                        Карточка доктора с детальным описанием, примерами работ и заслугами
                                    </div>
                                </div>
                                <div class="port-proj__item">
                                    <div class="port-proj__item-title">о клинике
                                        <div class="port-proj__item-line"></div>
                                    </div>
                                    <div class="port-proj__item-desc">
                                        Краткая история о достижениях, преимущесвтах клиник.  Для оформления были использованы фотографии и иконки
                                    </div>
                                </div>
                            </div>
                            <!-- END list -->
                        </div>
                        <!-- END block 2 -->

                        <!-- block 3 -->
                        <div class="port-proj__block">
                        </div>
                        <!-- END block 3 -->

                    </div>
                    <!-- END right -->

                </div>
                <!-- END project -->

                <!-- project 2 -->
                <div class="port-proj port-proj_2">

                    <!-- left -->
                    <div class="port-proj__left">
                        <div class="port-proj__img">
                            <img class="port-proj__img-img" src="/images/content/portfolio/project14/project2.jpg">
                        </div>
                    </div>
                    <!-- END left -->

                    <!-- right -->
                    <div class="port-proj__right">

                        <!-- block 1-->
                        <div class="port-proj__block">
                            <!-- title -->
                            <div class="port-proj__title">
                                СТРАНИЦА
                                УСЛУГИ
                            </div>
                            <!-- END title -->
                        </div>
                        <!-- END block 1-->

                        <!-- block 2 -->
                        <div class="port-proj__block">
                            <!-- list -->
                            <div class="port-proj__list">
                                <div class="port-proj__item">
                                    <div class="port-proj__item-title">ОПИСАНИЕ
                                        <div class="port-proj__item-line"></div>
                                    </div>
                                    <div class="port-proj__item-desc">
                                        Разработка детального описания услуги с упором на SEO
                                    </div>
                                </div>
                                <div class="port-proj__item">
                                    <div class="port-proj__item-title">врачи
                                        <div class="port-proj__item-line"></div>
                                    </div>
                                    <div class="port-proj__item-desc">
                                        Показаны врачи, которые выполнят данную услугу
                                    </div>
                                </div>
                            </div>
                            <!-- END list -->
                        </div>
                        <!-- END block 2 -->

                        <!-- block 3 -->
                        <div class="port-proj__block"></div>
                        <!-- END block 3 -->

                    </div>
                    <!-- END right -->

                </div>
                <!-- END project 2 -->

                <!-- project 3 -->
                <div class="port-proj port-proj_3">

                    <!-- left -->
                    <div class="port-proj__left">
                        <div class="port-proj__img">
                            <img class="port-proj__img-img" src="/images/content/portfolio/project14/project3.jpg">
                        </div>
                    </div>
                    <!-- END left -->

                    <!-- right -->
                    <div class="port-proj__right">

                        <!-- block 1-->
                        <div class="port-proj__block">
                            <!-- title -->
                            <div class="port-proj__title">
                                страница
                                о враче
                            </div>
                            <!-- END title -->
                        </div>
                        <!-- END block 1-->

                        <!-- block 2 -->
                        <div class="port-proj__block">
                            <!-- list -->
                            <div class="port-proj__list">
                                <div class="port-proj__item">
                                    <div class="port-proj__item-title">СЕРТИФИКАТ
                                        <div class="port-proj__item-line"></div>
                                    </div>
                                    <div class="port-proj__item-desc">
                                        Детальная карточка доктора с заслугами 
                                    </div>
                                </div>
                                <div class="port-proj__item">
                                    <div class="port-proj__item-title">работы «до И после»
                                        <div class="port-proj__item-line"></div>
                                    </div>
                                    <div class="port-proj__item-desc">
                                        Примеры выполненных работ
                                    </div>
                                </div>
                            </div>
                            <!-- END list -->
                        </div>
                        <!-- END block 2 -->

                    </div>
                    <!-- END right -->

                </div>
                <!-- END project 3 -->

                <!-- pages -->
                <div class="port-pages">
                    <div class="port-pages__title">
                        внутренние
                        страницы
                    </div>
                    <div class="port-pages__imgs">
                        <img class="port-pages__img" src="/images/content/portfolio/project14/pages.png"></div>

                </div>
                <div class="port-pages-bottom"></div>
                <!-- END pages -->

                <!-- adapt -->
                <div class="port-adapt">

                    <!-- title -->
                    <div class="port-adapt__title">
                        Адаптивная версия
                    </div>
                    <!-- END title -->

                    <!-- top -->
                    <div class="port-adapt__top">

                        <!-- list -->
                        <div class="port-adapt__list">
                            <div class="port-adapt__item">
                                <!-- img -->
                                <div class="port-adapt__item-img">
                                    <img src="/images/content/portfolio/project14/tablet.png" alt=""
                                         class="port-adapt__item-pic">
                                </div>
                                <!-- END img -->

                                <!-- desc -->
                                <div class="port-adapt__item-desc">
                                    <div class="port-adapt__item-title">Планшетная</div>
                                    <div class="port-adapt__item-px">768 px</div>
                                </div>
                                <!-- END desc -->

                            </div>
                            <div class="port-adapt__item">
                                <!-- img -->
                                <div class="port-adapt__item-img">
                                    <img src="/images/content/portfolio/project14/phone.png" alt=""
                                         class="port-adapt__item-pic">
                                </div>
                                <!-- END img -->

                                <!-- desc -->
                                <div class="port-adapt__item-desc">
                                    <div class="port-adapt__item-title">Мобильная</div>
                                    <div class="port-adapt__item-px">320 px</div>
                                </div>
                                <!-- END desc -->

                            </div>
                        </div>
                        <!-- END  list-->

                    </div>
                    <!-- END top -->

                    <!-- middle -->
                    <div class="port-adapt__middle">
                        Сделана адаптивная верстка для корректного отображения на всех устройствах
                    </div>
                    <!-- END middle -->


                    <!-- bottom -->
                    <div class="port-adapt__bottom">
                        <div class="port-adapt__proj">
                            <img src="/images/content/portfolio/project14/project-tablet.jpg" alt=""
                                 class="port-adapt__proj-tab">
                            <img src="/images/content/portfolio/project14/project-phone.jpg" alt=""
                                 class="port-adapt__proj-tab">
                        </div>
                    </div>
                    <!-- END bottom -->
                </div>
                <!-- END adapt -->

                <!-- team -->
                <div class="port-team">
                    <div class="port-team__title">Команда<br>проекта</div>
                    <div class="port-team__img">
                        <img class="port-team__img-img" src="/images/content/portfolio/team.png">
                    </div>
                    <div class="port-team__list">
                        <div class="port-team__col">
                            <div class="port-team__item">
                                <div class="port-team__item-title">Чумак Ирина</div>
                                <div class="port-team__item-prof">Менеджер проекта</div>
                            </div>
                            <div class="port-team__item">
                                <div class="port-team__item-title">Подольский Валентин</div>
                                <div class="port-team__item-prof">Дизайнер</div>
                            </div>
                            <div class="port-team__item">
                                <div class="port-team__item-title">Ефимов Максим</div>
                                <div class="port-team__item-prof">Front-end разработчик</div>
                            </div>
                        </div>
                        <div class="port-team__col">
                            <div class="port-team__item">
                                <div class="port-team__item-title">Абрамов Андрей</div>
                                <div class="port-team__item-prof">Back-end разработчик</div>
                            </div>
                            <div class="port-team__item">
                                <div class="port-team__item-title">Соколов Александр</div>
                                <div class="port-team__item-prof">Тестировщик</div>
                            </div>
                        </div>
                        <div class="port-team__col">
                            <div class="port-team__item">
                                <div class="port-team__item-title">Рожанский Виктор</div>
                                <div class="port-team__item-prof">Контент менеджер</div>
                            </div>
                            <div class="port-team__item">
                                <div class="port-team__item-title">Тихонов Павел</div>
                                <div class="port-team__item-prof">Маркетолог</div>
                            </div>
                        </div>
                    </div>
                </div>
                <!-- END team -->

            </div>
            <section class="port-bottom">
                <div class="bottom">
                    <div class="container2">
                        <div class="bottom__send">
                            <? include $_SERVER['DOCUMENT_ROOT'] . '/pages/blocks/bottom/send.php' ?>
                        </div>
<div class="why">
    <div class="why__img"><img class="why__img-img" src="/images/content/why.png"></div>
    <div class="why__title">Работаем на результат</div>
    <div class="why__content">
        <p>Для нас важно создать проект и привести его в идеально состояние. Мы заинтересованы создавать те проекты, которые нам не стыдно показать. К сожалению, в некоторых проектах мы не можем проявлять свой талант и креативный подход в полном объеме, так как отталкиваемся от жёстких требований заказчиков по цвету, расположению элементов на странице, 
		эффектов и размещению информации. Обращаясь к нам, вы получаете команду специалистов с большим опытом работы. Для нас нет невыполнимых задач, мы возьмемся за самые смелые ваши идеи и доведём их до финального этапа.</p>
		<br>
	</div>
</div>
                        <div class="bottom__footer">
                            <? include $_SERVER['DOCUMENT_ROOT'] . '/pages/blocks/bottom/footer.php' ?>
                        </div>
                    </div>
                </div>
            </section>
        </div>
    </main>
</main>
<!-- END main -->

<? include $_SERVER['DOCUMENT_ROOT'] . '/pages/bottom.php' ?>
